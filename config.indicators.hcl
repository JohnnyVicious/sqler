...
getallindicators {
    ...
    exec = <<SQL
          SELECT TOP 1000 * FROM indicators WHERE INDICATOR_EXCHANGE = :exchange AND INDICATOR_PAIR = :pair AND INDICATOR_PERIOD = :period AND INDICATOR_TYPE = :type AND INDICATOR_NAME = :name AND INDICATOR_VERSION = :version ORDER BY INDICATOR_TIMESTAMP DESC;
    	SQL
}
